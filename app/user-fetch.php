...
if (isset($_POST['draw'])) {
        ...
        if (!is_null($searchValue)) {
                $totalQuery = misc\mysql\query("SELECT COUNT(*) as total FROM `users` WHERE (`username` like ? or `hwid` like ? or `ip` like ? or `banned` like ? ) and app = ?", ["%" . $searchValue . "%", "%" . $searchValue . "%", "%" . $searchValue . "%", "%" . $searchValue . "%", $_SESSION['app']]);
        }
        else {
                $totalQuery = misc\mysql\query("SELECT COUNT(*) as total FROM `users` WHERE app = ?", [$_SESSION['app']]);
        }
        $totalRow = mysqli_fetch_assoc($totalQuery->result);
        $totalRecords = $totalRow['total'];

        // Get total count without filter
        $allRecordsQuery = misc\mysql\query("SELECT COUNT(*) as total FROM `users` WHERE app = ?", [$_SESSION['app']]);
        $allRecordsRow = mysqli_fetch_assoc($allRecordsQuery->result);
        $allRecords = $allRecordsRow['total'];
        ...
        while ($row = mysqli_fetch_assoc($query->result)) {
                ...
                $data[] = array(
                        "checkbox" => '<input type="checkbox" class="user-checkbox" value="' . htmlspecialchars($row['username']) . '">',
